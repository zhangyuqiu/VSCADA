...
void DataControl::receive_can_data(uint32_t addr, uint64_t data){
    ...
    for (uint i = 0; i < states.size(); i++){
        if(states.at(i)->primAddress == static_cast<int>(addr) && static_cast<uint32_t>(states.at(i)->value) == isolateData64(static_cast<uint>(states.at(i)->auxAddress),static_cast<uint>(states.at(i)->offset),data,states.at(i)->endianness)){
